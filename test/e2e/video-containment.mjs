...
await build({
  ...
  ` }, bundle: true, outfile: path.join(output, 'app.js'), platform: 'browser',
  define: { 'process.env': '{"NODE_ENV":"test"}' }, jsx: 'automatic',
});
await writeFile(path.join(output, 'index.html'), '<!doctype html><html><head><link rel="stylesheet" href="/styles.css"><style>body{margin:0}section{max-width:960px;margin:auto;padding:16px}h2{margin:12px}</style></head><body><div id="root"></div><script src="/app.js"></script></body></html>');
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    res.setHeader('Content-Type', file.endsWith('.mp4') ? 'video/mp4' : file.endsWith('.css') ? 'text/css' : file.endsWith('.js') ? 'text/javascript' : 'text/html');
    res.end(bytes);
...
try {
  for (const [name, browserType] of [['chromium', chromium], ['webkit', webkit]]) {
    ...
    catch (error) { results.push({ browser: name, skipped: String(error).split('\n')[0] }); continue; }
    try {
      for (const viewport of [{width:390,height:844}, {width:1440,height:900}]) {
        ...
        for (const frame of page.frames()) {
          for (const video of await frame.locator('video').all()) {
            ...
            await video.evaluate(el => el.readyState >= 2 ? undefined : new Promise((resolve, reject) => {
              el.addEventListener('loadeddata', resolve, {once:true});
              el.addEventListener('error', () => reject(new Error('Video failed')), {once:true});
            }));
            const details = await video.evaluate(el => {
              const rect = el.getBoundingClientRect();
              return {fit:getComputedStyle(el).objectFit,width:rect.width,height:rect.height,nativeWidth:el.videoWidth,nativeHeight:el.videoHeight};
            });
            assert.equal(details.fit, 'contain');
            assert.ok(details.width > 0 && details.height > 0 && details.nativeWidth > 0);
          }
        }
        assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false);
        await page.screenshot({path:path.join(output, `${name}-${viewport.width}.png`),fullPage:true});
        results.push({browser:name,width:viewport.width,passed:true,videos:20});
        await page.close();
      }
