...
create or replace view public.public_projects_view
...
create or replace view public.public_assets_view
with (security_invoker = true)
as select
  id,
  key,
...
create or replace view public.public_tags_view
with (security_invoker = true)
as select
  id,
  slug,
...
grant select on public.public_assets_view to anon, authenticated;
grant select on public.public_tags_view to anon, authenticated;
