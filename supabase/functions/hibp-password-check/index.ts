Deno.serve(async (req) => {
  try {
    ...
    const passwords = Array.isArray(payload?.passwords)
      ? payload.passwords
    ...
    const fetchPrefix = async (prefix, entries) => {
      ...
      for (const line of lines) {
        ...
        if (lineSuffix)
          map.set(
            lineSuffix.toUpperCase(),
            parseInt((countStr || '0').replace(/\D/g, ''), 10) || 0
          );
      }
      for (const e of entries) {
    ...
    const concurrency = 6;
    let idx = 0;
    const workers = new Array(Math.min(concurrency, prefixes.length))
      .fill(null)
      .map(async () => {
