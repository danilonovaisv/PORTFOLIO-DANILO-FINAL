...
description: Audita o sistema de motion de uma codebase e produz findings priorizados
  e planos autocontidos de implementação. Read-only sobre source code. Use para auditoria
  ampla e roadmap; para revisão localizada use review-animations e para implementação
  use animate/animate-expo.
metadata:
  portfolio-design-auditor-role: audit-planning
...
Executa auditoria ampla de motion, identifica problemas de maior impacto, separa ISSUE de OPPORTUNITY e produz planos de implementação suficientemente específicos para outro agente executar sem depender de gosto implícito.

## Quando usar

Use para 'audit motion', 'improve animations', 'make the app feel better' ou quando vários componentes/fluxos precisam ser analisados em conjunto. Carregue `references/AUDIT.md` para o procedimento e `references/PLAN-TEMPLATE.md` para planos executáveis.

## Quando não usar

Não aplicar fixes diretamente; não usar para uma única diff pequena; não inflar severidade; não transformar diferenças estilísticas em problemas; não criar plano sem mapear dependências e constraints reais.

## Formato / contrato de uso

**Entrada:** repository context, target scope, implementation evidence, design/motion system, relevant skills e constraints.

**Saída:** strengths to preserve, findings priorizados, evidence, affected areas, effort/risk, implementation plan, dependencies, acceptance criteria e validation plan.

**Pós-condição:** somente findings aprovados seguem para implementação; auditoria stale deve ser revalidada antes de executar.
...
The rule catalog with precise values lives in [AUDIT.md](references/AUDIT.md). The plan format lives in [PLAN-TEMPLATE.md](references/PLAN-TEMPLATE.md). Load them when you audit and when you write plans.
...
One plan per selected finding, using [PLAN-TEMPLATE.md](references/PLAN-TEMPLATE.md), written into `plans/` as `NNN-short-slug.md` (monotonic numbering; respect existing plans). Stamp each plan with the current commit (`git rev-parse --short HEAD`).
