---
name: apple-design
description: 'Aplica princípios de interfaces fluidas derivados de Apple design/WWDC
  como lente de auditoria e implementação: feedback, continuidade espacial, gestos,
  springs, momentum, interrupção, materiais, tipografia e reduced motion. Use seletivamente,
  sem copiar estética Apple.'
metadata:
  portfolio-design-auditor-role: principles
...
Fornece critérios para avaliar e construir interfaces responsivas, físicas e interruptíveis: feedback imediato, manipulação direta, continuidade espacial, momentum, soft boundaries, springs, profundidade, tipografia e reduced motion. No plugin, funciona como lente de qualidade, não como direção visual obrigatória.

## Quando usar

Use em gesture-driven UI, drag/swipe/sheets, spring motion, momentum, transitions interruptíveis, feedback, translucency/depth, typography e reduced-motion. Combine com evidência real do produto e preserve o design system existente.

## Quando não usar

Não usar para concluir que a interface deve parecer um produto Apple; não impor system fonts, glass ou springs sem problema real; não substituir accessibility standards, documentação do produto ou identidade visual; não inventar 'Apple rules' ausentes da referência.

## Formato / contrato de uso

**Entrada:** target, comportamento observado, evidência, linguagem visual/motion existente e problema a resolver.

**Saída:** princípio aplicável, evidência de aplicabilidade, impacto, recomendação, o que preservar, o que não copiar e método de validação.

**Classificação:** `APPLICABLE`, `NOT_APPLICABLE` ou `INSUFFICIENT_EVIDENCE`. Um princípio só deve gerar finding quando produzir valor acionável.
