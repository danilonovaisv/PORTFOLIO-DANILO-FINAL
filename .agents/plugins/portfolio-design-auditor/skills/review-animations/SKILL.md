...
description: Revisa motion existente ou uma mudança específica contra padrões de craft,
  performance, interação e accessibility. Read-only por padrão e focada em motion.
  Use para validação localizada; para auditoria ampla use improve-animations.
metadata:
  portfolio-design-auditor-role: validation
...
Revisa motion existente com uma barra alta de qualidade: propósito, origem, timing, easing/spring, interruption, exit, gesture behavior, performance e accessibility. Produz findings localizados e critérios claros de aprovação.

## Quando usar

Use para revisar uma diff, componente, interação ou implementação de motion específica, inclusive após `animate`/`animate-expo`. Consulte `references/STANDARDS.md` quando um finding exigir valores, regras ou referências precisas.

## Quando não usar

Não revisar código não relacionado a motion; não implementar feature nova; não usar como auditoria de codebase inteira; não reprovar apenas por preferência estética; não assumir runtime behavior sem evidência.

## Formato / contrato de uso

**Entrada:** target/diff, comportamento esperado, código relevante, design/motion language e acceptance criteria.

**Saída:** `PASS`, `PASS_WITH_NOTES`, `REQUIRES_REVISION` ou `BLOCKED`; findings com evidence, severity, recommendation e validation steps.

**Pós-condição:** aprovação exige que comportamento relevante de desktop/touch/responsive/interruption/reduced motion/performance tenha sido verificado ou marcado como não verificável.
...
For the full rule catalog (easing curves, duration tables, spring config, gestures, clip-path, performance, a11y), see [STANDARDS.md](references/STANDARDS.md). Load it whenever a finding needs a precise value or citation.
...
4. **Sub-300ms UI.** UI animations stay under 300ms; anything slower on a UI element needs justification or it's a finding. Per-element budgets live in [STANDARDS.md](references/STANDARDS.md).
...
Be specific and cite `file:line`. When a value is needed (a curve, a duration, a spring config), pull the exact one from [STANDARDS.md](references/STANDARDS.md) rather than approximating.
