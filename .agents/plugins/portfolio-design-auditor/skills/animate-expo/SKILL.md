---
name: animate-expo
description: Implementa motion e gestos em React Native/Expo com Reanimated, Gesture
  Handler, Expo Router e haptics quando aplicáveis. Use na implementação mobile nativa
  aprovada; para web use animate e para auditoria use improve-animations/review-animations.
metadata:
...
Implementa animações e gestos em React Native/Expo priorizando execução fluida no UI runtime, continuidade do gesto, velocity handoff, interrupção, comportamento em hardware real e degradação adequada.

## Quando usar

Use para press feedback, sheets, drag/swipe, screen transitions, gesture-driven UI, haptics e correções de stutter em apps Expo/React Native, após o objetivo da interação estar definido.

## Quando não usar

Não usar para web/PWA; não assumir hover; não validar qualidade apenas em simulator; não mover trabalho por frame para o React Native runtime quando puder permanecer no UI runtime; não introduzir dependências sem justificar impacto.

## Formato / contrato de uso

**Entrada:** target, plataforma, versões/dependências existentes, gesto ou transição desejada, constraints de device, reduced motion e acceptance criteria.

**Saída:** estratégia de thread/runtime, propriedades animadas, spring/timing, gesture lifecycle, velocity handoff, haptics quando justificadas, fallback e plano de teste em device.

**Pós-condição:** validar em hardware representativo quando possível; quando isso não for possível, marcar explicitamente `não verificado: dispositivo real`.
