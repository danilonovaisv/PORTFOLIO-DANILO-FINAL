...
description: Mapeia descrições vagas de motion para termos técnicos precisos e vocabulário
  compartilhado. Use para nomear efeitos, gestos e padrões antes de documentar findings,
  prompts ou tarefas; não use para implementar ou julgar qualidade.
metadata:
  portfolio-design-auditor-role: analysis-support
...
Converte uma descrição informal de movimento em um termo técnico consistente, com uma definição curta e alternativas quando houver ambiguidade. Serve como vocabulário comum entre auditor, designer e agente de implementação.

## Quando usar

Use quando um finding ou briefing descreve 'o efeito que...' sem nome estável, ou quando a terminologia precisa ser normalizada antes de criar acceptance criteria e tarefas.

## Quando não usar

Não usar para decidir se uma animação é boa, necessária ou acessível; não gerar implementação; não transformar uma analogia em regra de produto; não inventar terminologia quando a correspondência for incerta.

## Formato / contrato de uso

**Entrada:** descrição observada do movimento e, se disponível, contexto de interação.

**Saída:** melhor termo, definição de uma linha, até duas alternativas relevantes e uma nota de desambiguação quando necessário.

**Restrições:** preservar incerteza; se nenhum termo corresponder com confiança, responder `UNKNOWN` e descrever o comportamento literalmente.
