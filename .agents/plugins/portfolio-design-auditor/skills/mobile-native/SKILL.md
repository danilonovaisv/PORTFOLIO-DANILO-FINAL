---
name: mobile-native
description: 'Audita e corrige o platform layer de web apps em celulares para que
  pareçam nativos: viewport, touch, hover sticky, safe areas, browser chrome, scroll,
  tap behavior e real-device verification. Use para web mobile/PWA; para React Native
  use animate-expo.'
metadata:
  portfolio-design-auditor-role: audit-implementation
...
Identifica e corrige sinais de 'website dentro do telefone' ligados à plataforma web: viewport units, touch action, sticky hover, tap highlight, safe areas, input zoom, overscroll, selection, carousels e status/browser chrome.

## Quando usar

Use em auditorias ou implementações de web mobile/PWA, sheets, carousels, full-screen layouts e touch interactions. É especialmente útil quando algo funciona em desktop/devtools mas parece incorreto no aparelho.

## Quando não usar

Não usar como substituto de `animate` para craft de motion; não usar para React Native; não declarar experiência validada apenas com device emulation; não remover comportamentos nativos úteis sem motivo.

## Formato / contrato de uso

**Entrada:** target mobile, implementação, supported browsers/devices, viewport/scroll/touch behavior e acceptance criteria.

**Saída:** findings ou fixes por categoria, distinção entre code-verifiable e device-verifiable, safe-area/viewport strategy, touch/scroll behavior e plano de teste.

**Pós-condição:** itens dependentes de hardware devem ser marcados `não verificado: dispositivo real` até teste real.
