...
description: 'Varre uma interface para encontrar poucos pontos de alto valor onde
  motion pode melhorar continuidade, feedback ou compreensão, e rejeita o restante.
  Read-only: propõe receitas, não implementa. Use para descobrir oportunidades, não
  para revisar motion existente.'
metadata:
...
Procura oportunidades de motion com alto valor funcional e aplica um filtro de restrição: a maior parte dos elementos não precisa animar. Para cada oportunidade aceita, explica propósito, gatilho, propriedade, timing/curve e validação.

## Quando usar

Use quando a pergunta for 'onde motion ajudaria?' ou quando uma auditoria identificar ausência de feedback/continuidade, mas ainda não houver animação existente para revisar.

## Quando não usar

Não usar para encher a interface de movimento; não revisar animações existentes; não implementar; não propor motion que compita com conteúdo, aumente cognitive load, prejudique reduced motion ou não tenha motivo observável.

## Formato / contrato de uso

**Entrada:** target, fluxo, comportamento atual, design/motion language existente e constraints.

**Saída:** lista curta de oportunidades classificadas como `ACCEPT` ou `REJECT`; para cada `ACCEPT`, incluir propósito, gatilho, estados, propriedades, timing/curve, reduced motion, risco e acceptance criteria.

**Pós-condição:** ausência de oportunidades fortes é um resultado válido.
