...
Esta skill realiza auditorias técnicas exaustivas (SEO, Segurança, Performance) e **validação de integridade estrutural** baseada no arquivo mestre do projeto.

## ⚠️ Source of Truth (SSoT) - Regra de Ouro

Antes de analisar qualquer saída do `squirrel`, o agente deve ler:
`.context/DOCS-PORTFOLIO-PAGES/RULES-PORTFOLIO-STRUCTURE.md`

**A estrutura definida nesse arquivo é IMUTÁVEL.** - Se o `squirrel` detectar uma página que não está no documento, ela deve ser removida ou documentada primeiro.

- Se a ordem das sessões no código (ex: `05-METHOD` antes de `04-WHAT-I-DO`) divergir do arquivo de regras, a auditoria deve retornar **FALHA DE INTEGRIDADE**, independentemente do score técnico.

## 🛠️ Prerequisites & Setup

1. **Squirrel CLI:** Deve estar instalado ([squirrelscan.com](https://squirrelscan.com)).
2. **Context Path:** O arquivo de regras deve existir em `.context/DOCS-PORTFOLIO-PAGES/`.

## 🔄 Workflow de Auditoria Agêntica

### Fase 1: Calibração Arquitetural

1. Leia `RULES-PORTFOLIO-STRUCTURE.md`.
2. Mapeie a "Tabela Resumo Geral" para a memória de curto prazo do agente.
3. Verifique via terminal (`ls -R /app`) se a estrutura de diretórios respeita a numeração oficial (ex: `01-HEADER`, `02-HERO`).

### Fase 2: Execução Técnica (Squirrel)

Execute o comando de auditoria priorizando o ambiente **LIVE** para capturar a verdade real de renderização:

```bash
# Auditoria profunda com formato otimizado para Gemini
squirrel audit [https://portfoliodanilo.com](https://portfoliodanilo.com) --coverage full --format llm

```

### Fase 3: Validação Cross-Reference

Compare os achados do Squirrel com as regras do projeto:

- **SEO/Meta Tags:** Devem bater exatamente com o descrito na "Sessão 06 - PROJETO-SLUG" e outras.
- **ALPA V3 Integrity:** Verifique se os 10 blocos editoriais listados no SSoT estão presentes e sem erros de console.
- **Ghost Design:** Valide se os tokens de cor (#0048ff) e as regras de "silêncio visual" (sem scale/rotate agressivo) foram respeitados.

## 📋 Score Targets & Completion Criteria

| Score | Status    | Ação                                                        |
| ----- | --------- | ----------------------------------------------------------- |
| < 85  | REPROVADO | Focar em correções de Meta Tags e Performance R3F.          |
| 85-94 | PARCIAL   | Refinar acessibilidade (WCAG AA) e Mobile-First.            |
| 95+   | FIXED     | Só atingido se a **Integridade Arquitetural** estiver 100%. |

**Critérios de Conclusão:**

- ✅ Score Squirrel > 95.
- ✅ Zero divergências vs `RULES-PORTFOLIO-STRUCTURE.md`.
- ✅ Todas as mudanças de rota/sessão refletidas na documentação ANTES do commit final.

## 🚀 Parallel Execution with Sub-agents

Ao aplicar correções sugeridas pela auditoria:

1. **Agent A (Architect):** Valida se o fix não quebra a imutabilidade do SSoT.
2. **Agent B (Worker):** Aplica correções em lote (ex: `alt text` em 10 imagens) usando subagentes paralelos para ganhar velocidade.
3. **Agent D (QA):** Roda um `squirrel audit -C quick` após cada correção para validar o incremento no score.

## 🔍 Troubleshooting Comum

- **Comando não encontrado:** Verifique se `~/.local/bin` está no PATH.
- **Divergência Estrutural:** Se o código estiver "certo" e a documentação "errada", **PARE**. Peça ao usuário permissão para atualizar o `RULES-PORTFOLIO-STRUCTURE.md` antes de prosseguir.
