...
O diretório `.agents` está estritamente estruturado em 4 pilares geradores de autonomia agêntica:
...
## 🛡️ 2. Core Non-Negotiables (Ghost Era Principles)

**Agent, before writing any code, you MUST adhere to the following:**

1. **Ghost Design System:** Cores exatas (`#0048ff`, `#040013`), minimalismo editorial, sem cores vibrantes não autorizadas.
2. **Performance 60FPS:** "Zero Jank Policy." R3F (React Three Fiber) deve usar `<InstancedMesh>`, gerenciar texturas rigidamente e evitar leaks. Rejeite qualquer implementação visual que derrube a performance.
3. **Architecture Check:** O design de pastas Next.js `App Router` requer isolamento rígido entre Cliente (`"use client"`) e Servidor.
...
3. `.context/DOCS-PORTFOLIO-PAGES/RULES-PORTFOLIO-STRUCTURE.md` - Especificações imutáveis de cada página ANTES de modificar o front-end.
4. O `SKILL.md` ou `WORKFLOW.md` relacionado à tarefa atual.

## 👥 4. Available Personas & Specializations

- **The Commander (`orchestrator.md`)**: Lidera mudanças arquiteturais pesadas.
- **Spectral Artist (`frontend-specialist.md`)**: Engenharia 3D, Shaders e Movimento Fantasma (Ghost Motion).
- **Data Sentinel (`agent-supabase-audit.md`)**: Garantia de integridade do Banco e RLS em Supabase.
