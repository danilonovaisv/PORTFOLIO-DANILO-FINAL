...
async function checkSupabase() {
  ...
  if (error && error.code !== 'PGRST116') {
    // PGRST116 is just no rows, which implies connection worked
  ...
  if (fs.existsSync('supabase/config.toml')) {
    ...
    console.warn(
      '⚠️ supabase/config.toml not found. Local dev might be unconfigured.'
    );
  }

  console.log(
    "ℹ️ To verify Realtime, please run the full 'supabase-fixer' workflow which includes browser verification."
  );
}
