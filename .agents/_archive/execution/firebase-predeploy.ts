...
function checkFirebasePredeploy() {
    ...
    if (!['18', '20', '22'].includes(nodeVersion?.replace('>=', ''))) {
        console.warn(`⚠️ Warning: Node version ${nodeVersion} might not match Firebase Functions runtime. Recommended: 18, 20, or 22.`);
    }
    ...
    if (fs.existsSync(firebaseJsonPath)) {
        ...
        const hasRewrite = firebaseJson.hosting?.rewrites?.some((r: any) => r.source === '**' && r.function);
        const hasWebFrameworksBackend = !!firebaseJson.hosting?.frameworksBackend;

        if (hasRewrite || hasWebFrameworksBackend) {
            console.log("✅ Hosting configuration found (Rewrite or Frameworks Backend)");
        } else {
            console.warn("⚠️ No catch-all rewrite or frameworksBackend found. Ensure 'webframeworks' handles SSR.");
        }
    } else {
