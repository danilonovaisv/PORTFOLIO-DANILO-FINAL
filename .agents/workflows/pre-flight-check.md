...
4. Valide a performance das cenas WebGL e Next.js 15+ (Core Web Vitals e Bundle Size) utilizando Chrome DevTools MCP.
5. Execute o conjunto de testes unitários e de integração:
   `// turbo pnpm test`
6. Realize uma build de teste para validar a geração de páginas estáticas e hidratação segura:
   `// turbo /Users/danilonovais/.local/bin/node @node_modules/.bin/next build`
7. Se todos os passos passarem com sucesso (Status: PASSED), proceda com a tarefa de commit ou deploy via `/firebase-pipeline`.
