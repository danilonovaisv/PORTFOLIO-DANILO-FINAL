...
2. **Criação do Script de Limpeza Core (`next-cleanup.sh`):**
   Crie um arquivo na raiz chamado `next-cleanup.sh` com o exato conteúdo abaixo e dê permissão de execução (`chmod +x next-cleanup.sh`). *Não execute o script ainda.*
   ```bash
   #!/bin/bash
   cd "$(dirname "$0")" || exit
