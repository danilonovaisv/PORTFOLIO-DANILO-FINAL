# Storage Structure (Supabase) - V4

Esta documentação detalha a arquitetura padronizada (v4) para arquivos de mídia (`portfolio-media`) armazenados no Supabase, definindo regras rígidas de nomenclatura para evitar cache misses, estouro de egress, arquivos duplicados e downloads indesejados.

## A Estrutura Oficial V4

Todos os novos uploads devem obrigatoriamente seguir a seguinte estrutura de caminho:

```text
v4/<marca>/<projeto>/<subpastas-opcionais>/<filename>.<hash>.<ext>
```

- **`v4`**: Prefixo obrigatório da nova estrutura (substituindo o antigo v3).
...
**2. Quebra de Cache**
Nunca faça uploads com UPSERT na versão v4. Como a URL base agora altera se a imagem alterar, o frontend NUNCA usará URLs do tipo genérico com timestamps de cachebust (ex: `?t=12312312`) para prefixos `v4/`. Parâmetros mutáveis em query strings esgotam cotas de CDN e Egress rapidamente.
...
- **`npm run audit:v4:storage`**: Lê todo o bucket `portfolio-media` validando no Postgres os items que fugiram da regra Cache-Control ou estão salvos como /v1/, /v2/ e /v3/. Ele também alerta PNGs > 500KB e MP4s colocados onde deveriam ser imagens.
- **`npm run audit:v4`**: Verifica a codebase pra ver se há código hardcoded referenciando caminhos de pastas velhas ou v3. E intercepta autoPlays dentro de listagens ou grid.
