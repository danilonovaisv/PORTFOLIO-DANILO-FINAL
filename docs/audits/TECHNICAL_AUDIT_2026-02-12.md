...
### Featured Projects
- **Inconsistência**: atualização em tempo real com polling de fallback a cada 15s mesmo com dados em cache.
- **Evidência técnica**: `useRealtimeAsset` inicia polling e só para após SUBSCRIBED.
- **Impacto**: overhead de rede e wakeups em background, afetando bateria/perf.
- **Correção prática**: usar backoff exponencial e pausar polling em aba oculta (`document.visibilityState`).
- **Arquivos afetados**: `src/hooks/useRealtimeAssets.ts`.
