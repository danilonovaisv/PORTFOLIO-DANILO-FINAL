...
function validateCanvas(filePath: string): boolean {
  ...
  for (const node of canvas.nodes) {
    if (!/^[a-f0-9]{16}$/i.test(node.id)) {
      console.warn(
        `  ⚠️  Node ID format unexpected: ${node.id} (should be 16-char hex)`
      );
    }
  ...
  for (const edge of canvas.edges) {
    if (!nodeIds.has(edge.fromNode)) {
      console.error(
        `  ❌ Edge ${edge.id}: fromNode "${edge.fromNode}" not found`
      );
      valid = false;
    }
  ...
  if (valid) {
    console.log(
      `  ✅ Canvas valid — ${canvas.nodes.length} nodes, ${canvas.edges.length} edges`
    );
  }
  return valid;
}
...
function validateWikilinks(dir: string): void {
  ...
  for (const file of mdFiles) {
    ...
    for (const match of matches) {
      ...
      if (targetName.includes('.canvas') || targetName.includes('.base'))
        continue;
      ...
      if (
        ...
        console.warn(
          `  ⚠️  Broken link in ${path.basename(file)}: [[${targetName}]]`
        );
        broken++;
      }
    }
  ...
  if (broken === 0) {
    ...
    console.log(
      `  ⚠️  ${broken} broken wikilink(s) found — update 00-INDEX.md`
    );
  }
}
...
function generateCanvasNodeForNewModules(): void {
  ...
  for (const dir of srcDirs) {
    ...
    const targetFile = fs.existsSync(pageFile)
      ? `src/${dir}/page.tsx`
      : `src/${dir}`;
  ...
  if (newNodes.length > 0) {
    ...
    console.log(
      `  ✅ Added ${newNodes.length} new node(s) to architecture-map.canvas`
    );
  } else {
...
console.log(
  allValid
    ? '✅ Wiki-Brain sync complete — all checks passed.'
    : '⚠️  Wiki-Brain sync finished with warnings.'
);
