...
async function verifyAssetCorrections() {
  ...
  const invalidKeyAssets = assets.filter(asset => 
  ...
  const invalidPathAssets = assets.filter(asset => 
  ...
  const duplicatedPathAssets = assets.filter(asset => 
  ...
  const duplicatedKeyAssets = assets.filter(asset => 
  ...
  console.log(`Assets com chaves inválidas (começam com "updated_at:", "key:", etc.): ${invalidKeyAssets.length}`);
  console.log(`Assets com caminhos inválidos (contêm "updated_at:", "key:", URLs completas): ${invalidPathAssets.length}`);
  console.log(`Assets com caminhos duplicados (padrões conhecidos): ${duplicatedPathAssets.length}`);
  console.log(`Assets com chaves duplicadas (padrões conhecidos): ${duplicatedKeyAssets.length}`);
  ...
  const totalIssues = invalidKeyAssets.length + invalidPathAssets.length + 
                     duplicatedPathAssets.length + duplicatedKeyAssets.length;
  
  console.log(`\nTotal de problemas identificados: ${totalIssues}`);
  
  if (totalIssues === 0) {
    console.log('\n✅ Todas as correções de assets duplicados e inválidos foram bem-sucedidas!');
    console.log('A página admin deve agora exibir corretamente os assets válidos.');
  } else {
    console.log('\n⚠️  Ainda existem problemas com os assets que precisam ser corrigidos.');
    console.log('Considere executar novamente o script de correção: node scripts/fix-duplicate-assets.ts');
  }
}
