...
function main() {
    ...
    try {
        ...
        fs.writeFileSync(tempConfig, JSON.stringify({ ignore: ['.agent/**', 'artifacts/**'] }));
        
        // Executa o Knip e captura o JSON. Ignora erros de exit code
        knipOutput = execSync(`npx knip --config ${tempConfig} --reporter json`, { encoding: 'utf-8', stdio: ['ignore', 'pipe', 'ignore'] });
        
        // Clean up
        fs.unlinkSync(tempConfig);
    } catch (e) {
        // Clean up even if fail
        try { fs.unlinkSync('knip-temp-config.json'); } catch(err) {}
    ...
    const oldFiles = new Set(oldFilesContent.split('\n').filter(Boolean).map(f => f.trim()));
