...
function getSecurityStatus() {
  ...
  try {
    ...
    if (fs.existsSync(scanDir)) {
      cvesFixed = Math.min(
        totalCves,
        fs.readdirSync(scanDir).filter((f) => f.endsWith('.json')).length
      );
    }
  } catch {
    /* ignore */
...
function getSwarmStatus() {
  ...
  const progressData = readJSON(
    path.join(CWD, '.claude-flow', 'metrics', 'v3-progress.json')
  );
  if (progressData && progressData.swarm) {
    return {
      activeAgents:
        progressData.swarm.activeAgents || progressData.swarm.agent_count || 0,
      maxAgents: progressData.swarm.totalAgents || CONFIG.maxAgents,
      coordinationActive:
        progressData.swarm.active || progressData.swarm.activeAgents > 0,
    };
  }
...
function getSystemMetrics() {
  ...
  if (
    ...
    const fromPatterns =
      learning.patterns > 0
        ? Math.min(100, Math.floor(learning.patterns / 10))
        : 0;
    const fromVectors =
      agentdb.vectorCount > 0
        ? Math.min(100, Math.floor(agentdb.vectorCount / 100))
        : 0;
...
function getADRStatus() {
  const complianceData = readJSON(
    path.join(CWD, '.claude-flow', 'metrics', 'adr-compliance.json')
  );
  if (complianceData) {
    ...
    return {
      count: total,
      implemented: impl,
      compliance: complianceData.compliance || 0,
    };
  }
  ...
  for (const adrPath of adrPaths) {
    try {
      if (fs.existsSync(adrPath)) {
        ...
        const compliance =
          files.length > 0 ? Math.floor((implemented / files.length) * 100) : 0;
