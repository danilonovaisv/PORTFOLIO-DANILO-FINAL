...
When one rounded element sits inside another rounded element, the outer element's radius must be _larger_ than the inner element's radius — specifically, larger by the amount of padding between them.
...
Applying `border-radius: 50%` to a non-square element creates a pill. Applying it to a _square_ element creates a **circle**. Make sure you know which one you're creating.
