...
O _thumb video_ pertence à **Home Hero** (`.home-hero`), dentro de um wrapper identificado por classe **`.video-wrapper`**.
...
export default function HeroText() {
  ...
  return (
    <motion.div
      ...
      animate="show"
    >
      <span className="font-monospace text-xs uppercase tracking-widest mb-2">
        [Brand awareness]
      </span>
...
_Note:_ O uso de `pointer-events:none` impede interferência com o canvas; as tags `<h1>` e `<h2>` garantem estrutura semântica.
    ...
    <AnimatePresence>
      {!complete && (
        ...
        >
          ...
          <motion.p
            className="font-monospace text-xs uppercase tracking-widest mt-4"
            initial={{ opacity: 1 }}
...
export default function GhostScene() {
  ...
  useFrame(({ clock }) => {
    ...
    if (planeRef.current) {
      ...
      mat.uniforms.ghostPosition.value.copy(
        ghostRef.current?.position ?? new THREE.Vector3()
      );
    }
  });
  ...
  return (
    ...
    >
      ...
      <motion.button
        ...
        style={{
          opacity: isDesktop ? overlayOpacity : 0,
...
const GhostCanvasBackground = dynamic(() => import('./GhostCanvasBackground'), {
  ssr: false,
});
...
export default function HeroClient() {
  ...
  return (
    <section
      ref={heroRef}
      className="home-hero relative w-full h-[200svh] md:h-screen overflow-hidden bg-neutral-950"
    >
