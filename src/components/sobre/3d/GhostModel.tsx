...
interface GhostModelProps {
  ...
  scrollYProgress: { current: number };
...
export function GhostModel({
  ...
  useFrame((state) => {
    ...
    const scaleBoost =
      progress > 0.8 ? MathUtils.mapLinear(progress, 0.8, 1, 1, 1.1) : 1;
    ...
    const floating =
      Math.sin(state.clock.getElapsedTime() * floatingFreq) * floatingAmp;
    ...
    const rotSpeedBoost =
      progress > 0.8 ? MathUtils.mapLinear(progress, 0.8, 1, 1, 2.5) : 1;
    const targetRotY =
      (shouldReduceMotion ? 0 : pointerX.get() * 0.3) +
      Math.sin(state.clock.getElapsedTime() * baseRotSpeed * rotSpeedBoost) *
        0.15;
    ...
    if (meshRef.current.material) {
      ...
      const distortBoost =
        progress > 0.8 ? MathUtils.mapLinear(progress, 0.8, 1, 1, 1.8) : 1;
      (meshRef.current.material as any).distort = shouldReduceMotion
        ? 0
        : distortBase * distortBoost;
    }
  });
...
const supabaseUrlPreload =
  process.env.NEXT_PUBLIC_SUPABASE_URL ||
  'https://umkmwbkwvulxtdodzmzf.supabase.co';
useGLTF.preload(
  `${supabaseUrlPreload}/storage/v1/object/public/site-assets/about/beliefs/ghost-transformed.glb`
);
