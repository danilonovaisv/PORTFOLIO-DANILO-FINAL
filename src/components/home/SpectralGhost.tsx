// src/components/SpectralGhost.tsx
      ...
      `
        ...
        function createEyes() {
            ...
            const eyeMaterial = new THREE.MeshBasicMaterial({
                ...
                emissive: fluorescentColors[params.eyeGlowColor],
        ...
        function updateParticles(deltaTime: number) {
            particles.forEach((particle, index) => {
                if (particle.userData.velocity) {
                    ...
                    // Reduzir opacidade
                    particle.material.opacity -= params.particleDecayRate * deltaTime;
        ...
        function animate(timestamp: number) {
            ...
            ghostGroup.position.z += (targetZ - ghostGroup.position.z) * params.followSpeed;

            // Wobble
            ghostGroup.rotation.y = Math.sin(time * params.floatSpeed) * params.wobbleAmount;
            ghostGroup.rotation.x = Math.cos(time * params.floatSpeed) * params.wobbleAmount * 0.5;
            ...
            if (frameCount > 60 && !preloader.isComplete) {
        ...
        // GUI de controle (opcional, para desenvolvimento)
        ...
        // appearanceFolder.addBinding(params, "bodyColor", { label: "Body Color" }).on("change", (ev) => {
        //   ghostMaterial.color.set(ev.value);
        // });
        // appearanceFolder.addBinding(params, "glowColor", { label: "Glow Color", options: { ...fluorescentColors } }).on("change", (ev) => {
        //   ghostMaterial.emissive.set(fluorescentColors[ev.value]);
        // });
        // appearanceFolder.addBinding(params, "emissiveIntensity", { label: "Ghost Glow", min: 1.0, max: 10.0, step: 0.1 }).on("change", (ev) => {
        //   ghostMaterial.emissiveIntensity = ev.value;
        // });

        // // Controles dos olhos
        // const eyeFolder = pane.addFolder({ title: "Eye Controls", expanded: true });
        // eyeFolder.addBinding(params, "eyeGlowColor", { label: "Eye Glow Color", options: { ...fluorescentColors } }).on("change", (ev) => {
        //   leftEye.material.emissive.set(fluorescentColors[ev.value]);
        //   rightEye.material.emissive.set(fluorescentColors[ev.value]);
        // });
        // eyeFolder.addBinding(params, "eyeGlowIntensity", { label: "Eye Intensity", min: 1.0, max: 10.0, step: 0.1 }).on("change", (ev) => {
        //   leftEye.material.emissiveIntensity = ev.value;
        //   rightEye.material.emissiveIntensity = ev.value;
        // });
        ...
        // // Configurações de efeitos analógicos
        // const analogFolder = pane.addFolder({ title: "Analog Effects", expanded: false });
        // analogFolder.addBinding(params, "analogIntensity", { label: "Overall Intensity", min: 0, max: 3, step: 0.1 }).on("change", (ev) => {
        //   analogDecayPass.uniforms.uAnalogIntensity.value = ev.value;
        // });
        // analogFolder.addBinding(params, "analogGrain", { label: "Grain", min: 0, max: 3, step: 0.1 }).on("change", (ev) => {
        //   analogDecayPass.uniforms.uAnalogGrain.value = ev.value;
        // });
        // analogFolder.addBinding(params, "analogBleeding", { label: "Color Bleeding", min: 0, max: 3, step: 0.1 }).on("change", (ev) => {
        //   analogDecayPass.uniforms.uAnalogBleeding.value = ev.value;
        // });
        // analogFolder.addBinding(params, "analogVSync", { label: "V-Sync Roll", min: 0, max: 3, step: 0.1 }).on("change", (ev) => {
        //   analogDecayPass.uniforms.uAnalogVSync.value = ev.value;
        // });
        // analogFolder.addBinding(params, "analogScanlines", { label: "Scanlines", min: 0, max: 3, step: 0.1 }).on("change", (ev) => {
        //   analogDecayPass.uniforms.uAnalogScanlines.value = ev.value;
        // });
        // analogFolder.addBinding(params, "analogVignette", { label: "Vignette", min: 0, max: 3, step: 0.1 }).on("change", (ev) => {
        //   analogDecayPass.uniforms.uAnalogVignette.value = ev.value;
        // });
        // analogFolder.addBinding(params, "analogJitter", { label: "Jitter", min: 0, max: 3, step: 0.1 }).on("change", (ev) => {
                ...
                <div className="preloader-content">
                    <div className="ghost-loader">
                        ...
                        <svg width="80" height="80" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M50 10C30 10 10 30 10 50C10 70 30 90 50 90C70 90 90 70 90 50C90 30 70 10 50 10Z" fill="#00FFFF" />
