...
// --- SHADERS PERSONALIZADOS (Opcional, se não estiver no AnalogDecayPass) ---
// Se você não usar este shader personalizado diretamente no mesh e sim via AnalogDecayPass,
// pode remover esta parte e o extend abaixo.
/*
...
type TimeRef = React.MutableRefObject<number>;
...
const Scene = ({ mousePosition }: { mousePosition: MousePosition }) => {
  ...
  return (
    <>
      ...
      <GhostModel mousePosition={mousePosition} timeRef={timeRef} />
      <Environment preset="apartment" />
      <Fireflies />
      <AtmosphereVeil />
      <GhostEyes color={GHOST_CONFIG.eyeGlowColor} />
    </>
  );
};
...
const GhostCanvas = () => {
  ...
  return (
    <motion.div
      ...
      style={{
        background: GHOST_CONFIG.canvasBackground,
      }}
    >
      <Canvas
        gl={{
