...
export function useParticleSystem(params: GhostSceneParams) {
  ...
  const spawnParticle = useCallback((ghostPos: THREE.Vector3) => {
    let spawned = 0;
  ...
  }, [params.particleCount, params.particleCreationRate, params.particleDecayRate]);

  const update = useCallback((time: number, deltaTime: number) => {
