...
export function useGhostAnimate(
  ...
  particleSystem: any,
  input: { mouse: THREE.Vector2; hasReceivedMouseInput: React.MutableRefObject<boolean> },
  ...
  const update = useCallback((timestamp: number) => {
    ...
    if (!atmosphereMaterial || !analogDecayPass || !ghostMaterial || !eyes || !sharedFireflyLight || !camera || !renderer) return;
    ...
    const autoX = Math.sin(time * autoSpeed) * 9 + Math.cos(time * autoSpeed * 0.5) * 2;
    const autoY = Math.sin(time * autoSpeed * 0.7 + Math.PI / 2) * 6 + Math.sin(time * autoSpeed * 1.3) * 1.5;
    ...
    currentMovementRef.current = currentMovementRef.current * params.eyeGlowDecay + moveAmt * (1 - params.eyeGlowDecay);

    // Float & Pulse
    ghostGroup.position.y += Math.sin(time * params.floatSpeed * 1.5) * 0.03;
    ghostMaterial.emissiveIntensity = params.emissiveIntensity + Math.sin(time * params.pulseSpeed) * params.pulseIntensity;
